...
def my_model(encoder = 'VGG', input_size = (256, 256, 1), k_shot = 1, learning_rate = 1e-4):
    ...
    S_input  = layers.Input((k_shot, input_size[0], input_size[1], input_size[2]))  # None,1,224,224,3
    Q_input  = layers.Input(input_size)                                             # None,56,56,3
    S_mask   = layers.Input((k_shot, int(input_size[0]/4), int(input_size[1]/4), 1)) # None,1,224,224,1
    ...
    s_encoded = layers.TimeDistributed(layers.Conv2D(128, (3, 3), activation='relu', padding='same'))(s_encoded)   # None,1,56,56,128
    q_encoded = layers.Conv2D(128, (3, 3), activation='relu', padding='same')(q_encoded)                           # None,56,56,128
    ...
    ## Gussian filtering
    x1 = Sigma1_layer(s_encoded)      # None,1,56,56,128
    x2 = Sigma2_layer(s_encoded)      # None,1,56,56,128
    x3 = Sigma3_layer(s_encoded)      # None,1,56,56,128
    x4 = Sigma4_layer(s_encoded)      # None,1,56,56,128
    
    ## DOG Pyramid
    DOG1 = layers.Subtract()([s_encoded, x1])  # None,1,56,56,128
    DOG2 = layers.Subtract()([x1, x2])         # None,1,56,56,128
    DOG3 = layers.Subtract()([x2, x3])         # None,1,56,56,128  
    DOG4 = layers.Subtract()([x3, x4])         # None,1,56,56,128

    ## Global Representation
    s_1  = GlobalAveragePooling2D_r(S_mask)(DOG1)    # None,1,56,56,128
    s_2  = GlobalAveragePooling2D_r(S_mask)(DOG2)    # None,1,56,56,128
    s_3  = GlobalAveragePooling2D_r(S_mask)(DOG3)    # None,1,56,56,128
    s_4  = GlobalAveragePooling2D_r(S_mask)(DOG4)    # None,1,56,56,128

    ## Common Representation of Support and Query sample
    s_1  = common_representation(s_1, q_encoded)    # None,1,56,56,128
    s_2  = common_representation(s_2, q_encoded)    # None,1,56,56,128
    s_3  = common_representation(s_3, q_encoded)    # None,1,56,56,128
    s_4  = common_representation(s_4, q_encoded)    # None,1,56,56,128      
        
    ## Bidirectional Convolutional LSTM on Pyramid      
    s_3D   = layers.concatenate([s_1, s_2, s_3, s_4], axis=1) # None,4,56,56,128

    LSTM_f = layers.ConvLSTM2D(filters = 128, kernel_size=(3, 3), padding='same', return_sequences = False,   ## (None,56,56,128)
    go_backwards = False, kernel_initializer = 'he_normal')(s_3D)   

    LSTM_b = layers.ConvLSTM2D(filters = 128, kernel_size=(3, 3), padding='same', return_sequences = False,   ## (None,56,56,128)
    go_backwards = True,  kernel_initializer = 'he_normal')(s_3D)
    Bi_rep = layers.Add()([LSTM_f, LSTM_b])                 ## (None,56,56,128)
    ...
    final = layers.Conv2D(1, 1,   activation = 'sigmoid')(x)   ## None,224,224,1
...
model = my_model(input_size=(224,224,1))